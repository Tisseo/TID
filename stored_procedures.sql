...
CREATE OR REPLACE FUNCTION get_date_bounds_beetween_calendars_optimized(_first_calendar_id integer, _second_calendar_id integer) RETURNS date_pair
LANGUAGE plpgsql
    AS $$
    DECLARE
        _bit_mask bit varying;
    ...
    BEGIN
        ...
        IF(first_bounds.computed_start_date < second_bounds.computed_start_date) THEN
            max_start_date := second_bounds.computed_start_date;
        ELSE
            max_start_date := first_bounds.computed_start_date;
        END IF;
        IF(first_bounds.computed_end_date > second_bounds.computed_end_date) THEN
            min_end_date := second_bounds.computed_end_date;
        ELSE
            min_end_date := first_bounds.computed_end_date;
        END IF;
        _first_cal_bit_mask := getcalendarbitmask(_first_calendar_id, max_start_date, min_end_date);
        _second_cal_bit_mask := getcalendarbitmask(_second_calendar_id, max_start_date, min_end_date);
        -- Second get the resulted bitmask
        SELECT applybitmask(_first_cal_bit_mask, _second_cal_bit_mask, max_start_date, min_end_date, '&') INTO _bit_mask;
        -- Third get the bound of resulted bitmask
        SELECT * FROM detectmaskbounds(_bit_mask, max_start_date, ((min_end_date - max_start_date) + 1)::smallint) into _computed_date_pair;
        RETURN _computed_date_pair;
    END;
    $$;
COMMENT ON FUNCTION get_date_bounds_beetween_calendars_optimized(integer, integer) IS 'Optimized date bounds calculation (with minimal bitmask length)';
...
CREATE OR REPLACE FUNCTION purge_fh_data(_line_version_id integer default NULL) RETURNS VOID
    ...
    BEGIN
        ...
        DELETE FROM route_stop WHERE id NOT IN (SELECT DISTINCT(route_stop_id) FROM stop_time) AND route_id in (SELECT id FROM route WHERE line_version_id = _line_version_id);
